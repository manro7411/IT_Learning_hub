...
const AdministratorItems = [
    {name: "Dashboard" , icon: <Home size={20}/> ,path: "/administrator"},
    {name: "User management" , icon: <User size={20}/> ,path: "/administrator/user/management"},
    {name: "course Management" , icon: <BookIcon size={20}/> ,path: "/administrator/course/management"},
    {name: "Notification" , icon: <BookIcon size={20}/> ,path: "/administrator/notification"},
    {name: "System Log" , icon: <BookIcon size={20}/> ,path: "/administrator/system/log"}
]
export default function AdminsistratorBar () {
    ...
    return(
        ...
        <div>
          ...
          <nav className="space-y-4 text-sm">
            {AdministratorItems.map(({ name, icon, path }) => (
                <Link
                    key={path}
                    to={path}
